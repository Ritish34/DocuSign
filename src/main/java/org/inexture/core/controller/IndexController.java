package org.inexture.core.controller;

import com.docusign.esign.client.auth.OAuth;
import org.inexture.DSConfiguration;
import org.inexture.core.Locals;
import org.inexture.core.Session;
import org.inexture.core.User;
import org.inexture.core.utils.AccountsConverter;
...
public class IndexController {
    ...
    public String returnController(@RequestParam(value = ATTR_STATE, required = false) String state,
            ...
            @RequestParam(value = "envelopeId", required = false) String envelopeId, ModelMap model, HttpServletResponse response) {
        String site = "/end";
        response.setStatus(response.SC_MOVED_TEMPORARILY);
        response.setHeader("Location", site);
        return null;
    ...
    private OAuth.Account getDefaultAccountInfo(List<OAuth.Account> accounts) {
        ...
        if (StringUtils.isNotBlank(targetAccountId)) {
            OAuth.Account oAuthAccount = getAccountById(accounts, targetAccountId);
